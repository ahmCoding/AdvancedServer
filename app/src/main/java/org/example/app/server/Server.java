...
import org.example.app.utilities.Config;
...
public class Server implements ServerRemote {
    ...
    private void init() {
        logger = App.getLogger(this.getClass().getName());
    ...
    public void run() {
        if (!isRunning()) {
            ...
            handleRequestTask = new Thread(new ServerHandleRequestTask(myServer));
    ...
    public void handleRequests() {
        ...
        do {
            try {
                ...
                Thread delegatedTask = new Thread(new ServerExecuteCommandTask(clientSocket, myServer));
